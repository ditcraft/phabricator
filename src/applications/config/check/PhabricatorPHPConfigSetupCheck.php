...
final class PhabricatorPHPConfigSetupCheck extends PhabricatorSetupCheck {
  ...
  protected function executeChecks() {
    ...
    if (empty($_SERVER['REMOTE_ADDR'])) {
      ...
      $message = pht(
        'No REMOTE_ADDR is available, so ditCraft cannot determine the '.
        'origin address for requests. This will prevent ditCraft from '.
    ...
    if (!extension_loaded('mysqli')) {
      ...
      $message = pht(
        ...
        "\n\n".
        'ditCraft will work with the older extension, but upgrading to the '.
      ...
      $message = pht(
        ...
        'features (like support for query timeouts) which allow ditCraft '.
        'to interact better with the database.'.
        "\n\n".
        'ditCraft will work with the older driver, but upgrading to the '.
    ...
    if (ini_get($infile_key)) {
      ...
      $message = pht(
        ...
        'ditCraft to connect to a malicious MySQL server, you should '.
