...
commenting on a Differential revision), ditCraft can either send that mail
"From" the user's email address (like "alincoln@logcabin.com") or "From" the
'%s' address.

The user experience is generally better if ditCraft uses the user's real
...
email on behalf of the "From" domain. Practically, this means:
  ...
  - If you are doing an install for Example Corp and all the users will have
    corporate @corp.example.com addresses and any hosts ditCraft is running
...
a code review), ditCraft can either deliver one email to everyone (e.g., "To:
...
of each approach are:
  ...
  - One mail to everyone:
    ...
    - Sometimes people will "Reply All", which can send mail to too many
      recipients. ditCraft will try not to send mail to users who already
...
prefixed with "Re:". If you enable this option, ditCraft will add "Re:" to
...
If you enable `%s`, ditCraft uses "From" to authenticate users. You can
...
By default, ditCraft generates unique reply-to addresses and sends a separate
...
If you want to use a single mailbox for ditCraft reply mail, you can use this
and set a common prefix for reply addresses generated by ditCraft. It will
make use of the fact that a mail-address such as
`user123+D123+1hjk213h@example.com` will be delivered to the `us12321`
user's mailbox. Set this to the left part of the email address and it will be
prepended to all generated reply addresses.

For example, if you want to use `user123@example.com`, this should be set
to `user123`.
EODOC
));

    $address_description = $this->deformat(pht(<<<EODOC
When email is sent, what format should ditCraft use for user's email
...
ditCraft will automatically generate and use a suitable mailbox on the
inbound mail domain.
...
by ditCraft and some mail sent by users will bounce. If you point it at a
...
EODOC
    ...
    return array(
      ...
      $this->newOption(
        ...
        ->setSummary(
          pht(
            'Controls whether ditCraft sends one email with multiple '.
      ...
      $this->newOption('metamta.can-send-as-user', 'bool', false)
        ->setBoolOptions(
          array(
            ...
            pht('Send as ditCraft'),
          ))
        ->setSummary(
          pht(
            'Controls whether ditCraft sends email "From" users.'))
      ...
      $this->newOption(
        ...
        ->addExample('ditcraft.example.com', ''),
      ...
      $this->newOption('metamta.public-replies', 'bool', false)
        ...
        ->setSummary(
          pht(
            'ditCraft can use less-secure but mailing list friendly public '.
            'reply addresses.'))
        ->setDescription($public_replies_description),
      $this->newOption('metamta.single-reply-handler-prefix', 'string', null)
        ->setSummary(
          pht('Allow ditCraft to use a single mailbox for all replies.'))
      ...
      $this->newOption('metamta.user-address-format', 'enum', 'full')
        ...
        ->setSummary(pht('Control how ditCraft renders user names in mail.'))
